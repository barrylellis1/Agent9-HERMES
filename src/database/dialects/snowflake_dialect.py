"""
Snowflake SQL dialect - extracts SchemaContract from Snowflake view definitions.
...
class SnowflakeDialect(QueryDialect):
    """Schema contract extractor for Snowflake SQL dialect."""

    async def extract_schema_contract(self, view_name: str, view_sql: str) -> SchemaContract:
        """
        Extract complete schema contract from Snowflake view definition.
        ...
        try:
            ...
            self.logger.warning(f"Snowflake schema extraction error: {e}")
    ...
    def _extract_tables(self, normalized_sql: str) -> List[str]:
        ...
        # FROM clause: FROM [schema.]table [alias]
        ...
        join_pattern = r"(?:INNER\s+|LEFT\s+|RIGHT\s+|FULL\s+)?JOIN\s+(?:\"?[\w]+\"?\.)?\"?(\w+)\"?"
    ...
    def _extract_columns(self, normalized_sql: str) -> List[ColumnSchema]:
        ...
        # Pattern: word chars, dots (schema.table.col), optional AS alias
