...
class SupabasePrincipalProfileProvider(PrincipalProfileProvider):
    """Supabase-backed implementation of the principal profile provider."""
    ...
    def __init__(
        ...
        *,
        supabase_url: str,
        service_key: str,
        table: str = "principal_profiles",
        schema: str = "public",
        timeout: float = 10.0,
        source_path: Optional[str] = None,
    ) -> None:
        """
        Initialize Supabase principal profile provider.
        
        Args:
            supabase_url: Supabase project URL
            service_key: Service role key for admin access
            table: Table name (default: principal_profiles)
    ...
    async def load(self) -> None:
        """Fetch principal profiles from Supabase REST endpoint."""
        ...
        try:
            ...
            logger.error("Unexpected error fetching Supabase principal profiles: %s", exc)
        ...
        self._profiles_by_title.clear()

        loaded = 0
        for record in records:
            profile_id = (record.get("id") or "").strip()
            if not profile_id:
                continue

            try:
                # Build PrincipalProfile from Supabase row
                # Map Supabase columns to PrincipalProfile fields
                profile = PrincipalProfile(
                    id=profile_id,
                    name=record.get("name") or profile_id,
                    title=record.get("title") or "",
                    description=record.get("description"),
                    business_processes=record.get("business_process_ids") or [],  # Use normalized IDs
                    kpis=record.get("kpis") or [],
                    responsibilities=record.get("responsibilities") or [],
                    default_filters=record.get("default_filters") or {},
                    time_frame=record.get("time_frame") or {},
                    communication=record.get("communication") or {},
                    metadata=record.get("metadata") or {},
                )
            except Exception as profile_err:
                logger.warning(
                    "Skipping Supabase principal profile row due to validation error: %s",
                    profile_err
                )
                continue
        ...
        logger.info(f"Loaded {loaded} principal profiles from Supabase")


def create_supabase_principal_profile_provider(config: Dict[str, Any]) -> SupabasePrincipalProfileProvider:
    """
    Factory function to create a Supabase Principal Profile Provider.
    
    Args:
        config: Configuration dictionary with Supabase credentials
        
    Returns:
        SupabasePrincipalProfileProvider instance
    """
    return SupabasePrincipalProfileProvider(
        supabase_url=config["url"],
        service_key=config["service_key"],
        table=config.get("principal_table", "principal_profiles"),
