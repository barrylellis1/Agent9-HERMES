...
## Ratio Delta Additivity — `KTIsIsNot.deltas_are_contributions` (Aug 2026)
...
A ratio KPI's raw per-segment changes are **not additive**. DA already has a correct ratio path (the "bridge" in `_maps_for_level`): it fetches numerator and denominator separately per segment and reports `delta` as a revenue-**weighted** contribution (`rev_share * rate_change`), which *does* sum to the overall change. That path only runs when the KPI declares `metadata.kpi_type == "ratio"` plus `bridge_numerator_sql` / `bridge_denominator_sql`. The Lubricants Gross Margin % KPI declared neither, so DA fell through to raw deltas — **silently** — and the chart added them up.

**Fix:** `KTIsIsNot.deltas_are_contributions` (default `False`) is set only when **every** dimension went through the bridge, tracked by `_bridge_stats {levels, bridged}`. A partial run (bridge on one dimension, generic fallback on another) resolves to *not summable* rather than silently claiming additivity. Consumers must treat absent/False as "do not sum"; the frontend renders no total and explains why in a tooltip.

`_bridge_stats` is declared beside `kt` at the top of `execute_deep_analysis`, **not** inside the DPA branch that populates it — it is read unconditionally at the end of the method, and a definition nested in a conditional would raise `NameError` on every path where no KPI resolves.

**Open:** the alternative is to ask the warehouse for the total via `GROUP BY ROLLUP`, which needs no per-KPI metadata and no Python arithmetic — the data product owns the calculation. Pending a decision; the bridge stays as an optional *decomposition* either way. Tests: `tests/unit/test_ratio_delta_additivity.py`.
