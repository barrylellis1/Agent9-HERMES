...
`20260829120000_data_products_dimension_semantics.sql`) is now consulted before the legacy YAML scan —
...
## `_hierarchies_from_contract` is registry-first as of Phase 16 step 5 (Aug 2026)
- Found auditing every `yaml.safe_load` call site in this file before attempting to delete any of the 12 legacy contract YAML files (step 5's own precondition: "only safe once nothing reads them") — a THIRD YAML-only, live section of `views[].llm_profile` that the original Phase 16 finding table never catalogued (it only listed `dimension_semantics`). Unlike `column_aliases` (step 4, confirmed practically moot for every real client), this one has real, active behavioural impact: `execute_deep_analysis`'s `_hierarchies_from_contract()` closure sets `used_hierarchical = True` whenever the returned dict is non-empty, switching DA from flat dimension ranking to the hierarchical-drill analysis path.
- New `DataProduct.dimension_hierarchies: Dict[str, List[str]]` field (migration `20260829150000_data_products_dimension_hierarchies.sql`) and a new `_hierarchies_from_registry(kpi_name, client_id)` instance method, mirroring `_dims_from_registry` exactly — resolves `data_product_id` via `_lookup_kpi_scoped`, reads the registry field, non-fatal on any failure. The `_hierarchies_from_contract` closure (still local to `execute_deep_analysis`, capturing `plan`) tries this first, only falling back to the YAML scan when the registry result is empty.
- **Only genuinely live for hess among the three real clients** — `hess_financials.yaml` declares `dimension_hierarchies`; `lubricants_star_schema.yaml`/`lubricants_snowflake.yaml` never did. That means an empty result reads two different ways depending on the client: "not yet migrated" for hess, or "genuinely has none" for lubricants/apex_lubricants — both correctly fall through to an equally-empty YAML scan, so behaviour is unchanged either way; the distinction only matters for understanding *why* it's empty, not for what the code does.
- Verified live against the real bootstrapped registry (no mocks): hess's `gross_margin_pct` returns the seeded `{geography, segment, financials}` dict; lubricants' returns `{}` as expected. 7 new unit tests (`tests/unit/test_da_dimension_hierarchies.py`) cover `_hierarchies_from_registry` directly (the closure itself isn't independently unit-testable without invoking the whole method) — populated result, non-list values dropped not raised, empty-reads-as-empty for a client that genuinely has none, and non-fatal degradation on every failure mode `_dims_from_registry`'s own tests already cover.
- `onboard_client.py`'s `_DP_COLS` gained `dimension_hierarchies` (7th instance of the explicit-allow-list trap this session, added proactively).
- **Not resolved by this**: `_get_exposed_columns` (a fourth YAML-only `views[].llm_profile` section, `exposed_columns`) and `_contract_path_for_kpi`/`_dims_from_contract`'s YAML fallback for apex_lubricants/bicycle (dimension_semantics, step 1) both remain genuinely live — none of the 12 YAML files are safe to delete yet. See `DEVELOPMENT_PLAN.md`'s Phase 16 step 5 write-up for the full 14-call-site audit and what's still blocking file deletion.

## `_dims_from_contract`/`_hierarchies_from_contract` are registry-ONLY — the YAML fallback is gone (Phase 16 step 5, 2026-08-30)

The two entries above describe the registry-*first* state. Closed out the same phase, same day: once `dimension_semantics` was backfilled for hess/apex_lubricants and `exposed_columns` was migrated (see `A9_Data_Product_Agent_card.md`), all 4 real data products (`dp_lubricants_financials`, `dp_hess_financials`, `dp_lubricants_snowflake`, `fi_star_schema`) had complete registry records — verified live via direct query, not assumed. That made the YAML-contract fallback provably unreachable for every real client, so it was deleted rather than left as inert code:

- `_contract_path`/`_contract_path_for_kpi` (the YAML-contract-file resolvers) — **deleted**. Their only callers were the fallback branches removed in the same pass.
- `_dims_from_contract`'s YAML-scan branch — **deleted**. The method now returns whatever `_dims_from_registry` yields, full stop; `[]` when a data product has nothing declared, same result the YAML fallback produced for an unmigrated client before.
- `_hierarchies_from_contract`'s YAML-scan branch — **deleted**. The closure is now a one-line passthrough to `_hierarchies_from_registry`.
- The 8 legacy contract YAML files themselves (`src/registry_references/data_product_registry/data_products/*.yaml` — turned out to be 8 on disk, not the 12 the original 2026-08-10 finding counted) were deleted in the same commit.
- New architecture-lint rule (`scripts/architecture_lint.py` + `tests/architecture/test_architecture_compliance.py`) bans `yaml.safe_load(` in `src/agents/**` going forward, making CLAUDE.md rule 6 enforceable rather than aspirational (Phase 16 step 6). Two remaining call sites outside this cleanup's scope (`business_context_loader.py`'s SF debate-persona loader, `a9_llm_service_agent.py`'s dead ad-hoc-NL-to-SQL profile parsing) are allow-listed inline with `# arch-allow-yaml-fallback` and flagged in the lint script's own comment for anyone auditing rule 6 next — neither is KPI/principal/data-product/business-process registry data in rule 6's sense.

**One data product's registry fields are still empty** (`dp_lubricants_sales`, the 5 Sales KPIs) — checked before deleting anything: none of the 8 YAML files' `metadata.id` ever matched it either, so `_contract_path_for_kpi` always returned `""` for it and `_dims_from_contract` already yielded `[]` before this change. Deleting the fallback changes nothing for this data product — same empty result, same reason (nothing declared anywhere), not a new regression. Backfilling it is a separate, pre-existing gap, not part of this cleanup.

Verified: 1478 unit tests pass (1484 before this pass, net −6 — several tests that pinned "falls back to YAML scan" behaviour were rewritten to feed `_dims_from_registry`/`_hierarchies_from_registry` directly via mock instead of writing a temp YAML fixture and patching the now-deleted `_contract_path_for_kpi`; a few YAML-only fallback tests had no registry-only equivalent worth keeping and were removed rather than contorted).
