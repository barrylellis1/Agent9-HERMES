...
## `register_data_product` now writes `dimension_semantics` — Phase 16 Onboarding item O3 (Aug 2026)
- Found by actually driving the real Admin Console wizard against hess's live SQL Server database (`decision-studio-ui/tests/e2e/live-onboarding-hess-test.spec.ts`, a throwaway `dp_hess_financials_test` registration, deleted after inspection) rather than trusting the 2026-08-10 audit's characterization: schema profiling already tags every column `'dimension'`/`'measure'`/`'identifier'`/`'time'` via `_infer_semantic_tags` (the frontend already reads this back for `KPIAssistantChat`'s `schemaMetadata.dimensions`), but `register_data_product`'s `DataProduct(...)` construction never passed it through at all — a data product registered through the wizard got `dimension_semantics=[]` regardless of what was profiled, meaning `A9_Deep_Analysis_Agent._dims_from_contract` would have nothing to analyse.
- Fixed: new `_synthesize_dimension_semantics(tables)`, mirroring `_synthesize_time_dimensions`'s existing structure exactly — prefers a `FACT`-role table's columns first (same precedence rule), collects every column tagged `'dimension'`, deduplicates across tables, preserves profiled order. **Deliberately does NOT apply `_keep_contract_dim`'s ban-list** (flags, `_id`, `version`, `transaction_date`, ...) — that filter already runs uniformly at READ time in `A9_Deep_Analysis_Agent._dims_from_contract` regardless of whether the list came from the registry or the legacy YAML scan; duplicating it here would just be a second copy of the same rule to keep in sync, and a real risk of the two drifting apart.
- Verified live against hess's real schema: registered `dimension_semantics` closely matched hess's own manually-seeded list (`segment_name`, `basin_name`, `asset_name`, `country`, `business_unit`, `account_type`, `account_category`, `account_name`, ...) — the profiled set additionally included `version`/`currency` (harmless: `version` gets stripped by DA's ban-list at read time as designed; `currency` is a legitimate, if unused, dimension) and correctly omitted `fiscal_year`/`fiscal_period` (tagged `'time'`, not `'dimension'` — those belong in `time_dimensions`, not here).
- **A separate, pre-existing bug found and cleaned up in the same investigation, NOT fixed** (out of scope for O3): every "Schema Discovery" wizard step calls `orchestrate_data_product_onboarding` unconditionally, which calls `register_data_product` unconditionally, with `data_product_id='temp_discovery'` — meaning a junk `temp_discovery` row gets registered into Supabase for whichever client is targeted on every discovery run. Found two stray rows this way (`brookshire_brothers` from 2026-07-24, `hess` from this session's testing); both deleted directly, but the wizard itself still does this on every future discovery step. Flagged for a future fix, not addressed here.
- Tests: `tests/unit/test_data_product_agent_schema_generation.py` — empty-when-nothing-tagged, profiled-order preserved, FACT-table-first precedence (mirrors the existing time-dimensions tests), cross-table dedup, and an explicit test pinning that the ban-list is NOT applied here. `register_data_product`'s existing end-to-end wiring test extended to assert `dimension_semantics` lands in the registry entry; a new test covers the empty-list case registering cleanly.

## Onboarding now detects `measure_semantics` from live data — Phase 16 Onboarding item O2 (Aug 2026)
- The piece that would have caught Hess's original sign bug (COGS re-negated, adding cost to revenue — Phase 16 step 2/3) **at onboarding time**, not by a manual audit weeks later. Unlike O1 (`time_dimensions`) and O3 (`dimension_semantics`), which are pure functions over already-profiled column metadata, this genuinely needed new information: sign convention is a fact about the *data*, not the schema, so it requires a live query during profiling.
- New `_pick_sign_detection_columns(profile)`: identifies the `(type_column, amount_column)` pair to check, or `None`. Deliberately narrow — only fires when a column's name plausibly means "account type" (`'accounttype'` as a substring once underscores/spaces are stripped); a generic categorical column (`product_category`, `channel_type`) is **not** a safe guess, since sign convention is a finance-domain concept specific to account classification, not categorical columns in general. Prefers a measure column literally named `amount`; falls back to the first measure-tagged column.
- New `_detect_measure_sign(manager, source_system, table, type_column, amount_column, settings)`: runs `SELECT type_column, SUM(amount_column) GROUP BY type_column` against the live source (one dialect branch per backend, same shared-implementation shape as `_sample_distinct_values`), then derives `stored_sign[value] = "negative" if total < 0 else "positive"` per distinct value. **A value whose total is exactly zero is dropped, never guessed** — genuine ambiguity must not become a fabricated convention. Returns `None` (not an empty/default convention) when nothing conclusive comes back.
- New `_populate_measure_sign_convention`, wired into `_profile_table` right after `_populate_categorical_sample_values` (same call site, same best-effort posture) — sets the new `TableProfile.detected_measure_sign` field during profiling, when a live manager connection already exists.
- New `_synthesize_measure_semantics(tables)` in `register_data_product`, mirroring `_synthesize_dimension_semantics`'s FACT-table-first precedence — but returns a single dict or `None` rather than a list, since a data product has ONE sign convention, not several to merge across tables.
- **Verified live against hess's real SQL Server database** (`live-onboarding-hess-test.spec.ts`, throwaway `dp_hess_financials_test` registration, deleted after inspection): detected `{"Revenue": "positive", "COGS": "negative", "SGA": "negative", "Other": "negative"}` — an **exact match** to the manually-seeded convention this session's earlier steps (2/3) declared and used to fix Hess's actual KPI bugs. This is the strongest possible confirmation the detection logic is correct, not just plausible.
- Tests: `tests/unit/test_data_product_agent_schema_generation.py` — column-picking heuristic (finds the pair, rejects generic categorical columns, no-measure-column returns `None`, prefers literal `amount`), live-query detection per dialect (SQL Server/Snowflake query shape, zero-total dropped, all-inconclusive returns `None`, unknown backend short-circuits without querying, query exceptions swallowed), the profiling wrapper (sets the field / no-ops when no candidate columns, spying on `_detect_measure_sign` rather than re-testing it), and `_synthesize_measure_semantics`'s precedence. `register_data_product`'s wiring test extended to assert `measure_semantics` lands in the registry entry from a simulated `detected_measure_sign`; the empty-case test extended to assert `None` when nothing was detected.

### Human confirmation added the same day (closing the gap above)
- The original honesty note here said this field was written with no human confirmation, same posture as O1/O3 — and flagged that as a bigger deal for `measure_semantics` than for dimension ordering, since a wrong value here reproduces a real, silent numeric bug (the exact one that broke Hess). User agreed this needed closing rather than staying an accepted gap.
- New `confirm_measure_semantics(request: ConfirmMeasureSemanticsRequest)`: fetches the already-registered `DataProduct`, validates `client_id` ownership (same fail-loud pattern as `sync_related_business_processes` — never let a confirm call re-parent or write into a different tenant's record), overwrites `.measure_semantics` with the admin-confirmed (possibly corrected) value, upserts. New route `POST /data-product-onboarding/confirm-measure-semantics` (`workflows.py`), dispatched through the generic `execute_agent_method` like every other onboarding step.
- Frontend (`DataProductOnboardingNew.tsx`): the Metadata Analysis step now extracts `detected_measure_sign` from the same workflow response that already carries the inspection result (the `register_data_product` step's `details.registry_entry.measure_semantics` — that step already ran as part of the same call), and — when non-empty — renders a **required confirmation card** instead of advancing straight to KPI Definition: each detected `stored_sign` value as an editable positive/negative dropdown, plus a "Confirm Sign Convention & Continue" button that POSTs whatever the admin actually approved (not necessarily what was auto-detected, if they changed anything) to the new endpoint before advancing. When nothing was detected, the wizard behaves exactly as before — no gate appears.
- **Live-verified end to end** against hess's real SQL Server database: the card rendered showing `COGS`/`SGA`/`Other` = negative and `Revenue` = positive (screenshot captured), confirming as-detected persisted that exact value via the new endpoint — not just the pre-existing auto-write from `register_data_product`.
- 4 new unit tests (`confirm_measure_semantics` persists the confirmed value, an admin's correction overrides the original detection, client_id-mismatch rejection, not-found returns an error rather than raising). No React/TSX test suite exists in this codebase (no jest/vitest configured) — the UI side is covered by the live Playwright run above, not a unit test.

## `_get_exposed_columns` is registry-first as of Phase 16 step 5 (Aug 2026)
- Same "one contract fact, one place" pattern as `_get_contract_column_aliases` (step 4): `DataProduct.exposed_columns` is a new field, `Dict[str, List[str]]` keyed by lowercased view name (not a flat list) because the pre-existing YAML-scan code already looks up by view name with a fallback to `FI_Star_View` when the requested one isn't found — every seeded contract today declares exactly one view, but the shape stays faithful to what the code has always supported.
- `_get_exposed_columns(view_name, data_product_id=None)` gained a second parameter; tries the registry record first, only scanning the legacy YAML contract when the registry has nothing for that view. The in-memory cache (`_view_exposed_columns_cache`) is now keyed by `(data_product_id, view_name_lower)`, not view name alone — two data products can't collide on an identically-named view.
- **Found and fixed the same cross-tenant bug shape a third time**: the YAML fallback called `self._contract_path()` with no argument, always resolving to the bicycle default contract regardless of which data product was actually being asked about — same shape already fixed for `_dims_from_contract` (step 1) and `_get_contract_column_aliases` (step 4). Fixed by threading `data_product_id` through the method's single call site inside `_resolve_attribute_name` (`getattr(kpi_definition, 'data_product_id', None)` — `_resolve_attribute_name` already receives `kpi_definition`, so no change was needed at its own 7 call sites inside `_generate_sql_for_kpi`).
