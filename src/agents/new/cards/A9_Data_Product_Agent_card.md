...
- **Honesty note, matching O1's own gap**: this writes the detected convention directly, with no human confirmation step — same posture O1/O3 already ship with. Given this field is exactly what caused a real, silent numeric bug once already, that gap matters more here than for dimension ordering; not closed in this pass (would need a real UI confirmation screen, a materially bigger lift than the detection logic itself) — tracked in `DEVELOPMENT_PLAN.md`'s O2 write-up, not silently accepted as good enough.
