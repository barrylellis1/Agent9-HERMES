...
class A9_Principal_Context_Agent:
    ...
    async def _load_principal_profiles(self):
        ...
        try:
            ...
            if not self._principal_provider:
                self.logger.warning("Principal profile provider not available, using default profiles")
                self._load_default_profiles()
                return
            ...
            if not profiles:
                self.logger.warning("No principal profiles found in registry, using defaults")
                self._load_default_profiles()
                return
    ...
    def _load_default_profiles(self):
        ...
        # Create default profiles for key roles
        self.principal_profiles = {
            "cfo": {
                "role": "CFO",
                "business_processes": [
            ...
            "vp_finance": {
                ...
                "default_filters": {"region": "Americas"},
                "decision_style": "Analytical",
                "communication_style": "Detailed"
            }
        }
        self.logger.info(f"Loaded {len(self.principal_profiles)} default principal profiles")
