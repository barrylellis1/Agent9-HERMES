...
| **Reframe re-launch + framing lineage** — `docs/architecture/reframe_relaunch_and_lineage_design.md` (design note, not built). Found live 2026-08-22: reframing a KPI doesn't re-run DA — `generate_scqa_for_frame` reconstructs KT/change-points from the *original* KPI's `da_output`, only the narrative text changes, so solutions end up dressed in the new KPI's language while grounded in the old one's evidence. Phase 19's own plan named this and specified a disclosure string that was never actually shipped. Proposes a new-window/URL-param entry point (`DeepAnalysisRequest` needs only `kpi_name`, no situation-card dependency) rather than the deferred SA-provenance fix; the new window's framing gate must run fresh at each hop, not be skipped (each KPI's own causal neighbourhood is a genuinely new question); names the real gap — nothing links successive framing decisions into one chain, proposes `reframed_from_id` on `Assumption` so VA's eventual theory-layer capture sees the whole lineage. | Disclosure text: cheap, ship any time. Everything else: one coherent piece of work, not scheduled |
