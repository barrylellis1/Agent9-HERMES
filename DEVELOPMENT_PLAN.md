...
| **DQ L1 framing signal** — `docs/architecture/dq_l1_framing_signal_design.md` (design note, deliberately not ready to ship). `score_run()` never reads the framing gate's persisted decision; L1 infers "frame examined" from vocabulary-scanning SF prose instead. The obvious fix (score off `framing_choice` directly) is a real trap — conflates "objective examined" with "solutions structurally widened," risks inflating the pass rate as an artifact of this session's own causal-graph work. Requires a held-out validation against the pre-existing corpus (runs this session never touched) before adopting anything. | Validation run first; scoring-mechanism change only after |
