...
| **Causal edge direction + magnitude** — `docs/architecture/causal_edge_direction_and_magnitude_design.md` (design note, not built). Found live 2026-08-20: a Net Revenue framing gate offered COGS as an alternative objective with no real relationship to it — `KPIRelationship` has no field for which KPI causes which, so a 2-hop traversal walked a known edge backward. Proposes `causal_direction` (filtered only at `_build_framing_prompt`, SA's undirected BFS use is untouched) + `magnitude_category`/`magnitude_coefficient` (mirrors `confidence`'s categorical-not-a-float discipline; a real coefficient still requires `provenance="va_validated"`, same guardrail as `intervention_tested`). The direction piece specifically is small and additive (one field, a per-client edge backfill, one consumption-site filter) — candidate for sooner than the other two rows here, unlike the `magnitude_coefficient` half which is blocked on a Granger implementation that doesn't exist. | Direction: candidate for before-demo if time allows; magnitude/curve: deferred |
