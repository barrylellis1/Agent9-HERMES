...
| **UI defects found driving the live app (2026-08-22)** | Playwright pass against real lubricants data; zero console errors, so all of these are design/data defects not JS failures. Full detail in `docs/architecture/ui_refinement_plan.md` §8. Demo-blocking: `/portfolio` renders a raw developer error on direct navigation ("No principal ID provided. Add `?principal=cfo_001` to the URL") and `Test Probe / Test Title` is a selectable identity on the login screen. Small + unambiguous: `ANALYZE →` is hover-gated so the primary action is invisible on every card and unreachable on touch; a stray `0` renders in the Action Center (`ProblemRefinementChat.tsx:451-452` — a `\|\|` chain of `.length` values evaluates to `0` and React renders it). Design: the big % renders red (severity) above a green sparkline (direction) on the same card — one colour channel doing two contradictory jobs; Variance Breakdown shows the same `+$5.5M` on all seven dimension rows with raw UPPER_SNAKE_CASE column names. Latent: `KPITile.tsx:118-131` fabricates a 9-point quadratic sparkline from a single `percent_change` when `monthly_values` is missing, styled identically to real data — probed 0 of 15 firing today, undetectable if it ever does. |
