...
- Theory layer (Phase 17) — data models all BUILT 2026-08-30, exhibit dev-only. `kpi_decompositions` (arithmetic parentage), `ports` (external drivers), `kpi_relationships.basis` (identity vs. causal estimate), KPI semantic contract §3 (`additive_across_dimensions` et al.), plus two VA write-backs: assumption grading (`VALIDATED→held` / `FAILED→falsified`) and causal-edge confirmation (`intervention_tested` on a VALIDATED verdict that traces to the mechanism claimed at approval). The exhibit itself is at `/dev/theory-layer`, **deliberately unlinked from all nav** — its Causal Edges density gate (confirmed edges must outnumber unconfirmed) is unpassed at 0 tested edges, and clears only through accumulated VA verdicts over real use, never by seeding. Registry data not yet synced to production.
