...
- **Status (corrected 2026-08-30): APPLIED and in use.** ~~Schema designed and unit-tested (`tests/unit/test_theory_layer_causal_schema.py`), migration file written, NOT applied to any database (local or production). Held uncommitted per explicit instruction — apply only when there's a concrete producer (accretion pipeline) or consumer (SF Stage D input contract) ready to use it, not ahead of either.~~ That line was stale. Checked directly against the database on 2026-08-30 before acting on it: `supabase_migrations.schema_migrations` records version `20260723`, the `assumptions` table and its RLS policy exist live, and later migrations (`20260731_assumption_grading_fields.sql`, the framing columns) have already extended it. The gating condition it described was met and passed some time ago — **the producer and consumer both now exist**: SF HITL approval pre-registers a solution's bets (`src/api/routes/workflows.py`), and VA grades them at verdict time (`_grade_assumptions_from_verdict`, Phase 17 T3, 2026-08-30). Nothing needed applying for T3; only the write-back code was missing.
- **Assumption grading (the P1 write-back this section anticipated) — BUILT 2026-08-30.** `VALIDATED → held`, `FAILED → falsified`, `PARTIAL`/`MEASURING` left `active` (a partial outcome does not say *which* assumption held). Never overwrites an already-graded row. **Honest scope:** this is a SOLUTION-level verdict cascaded to every linked assumption, not independent per-assumption mechanism grading — `DEVELOPMENT_PLAN.md`'s "VA returns roughly one bit per solution" limitation still stands, and closing it needs the decomposition model's mechanism mapping.
...
> **Built 2026-08-30 (Phase 17 Stage 6) — as a dev-only prototype, and in a different shape than sketched above.** `decision-studio-ui/src/components/theory/TheoryLayerExhibit.tsx`, route `/dev/theory-layer`, deliberately unlinked from every nav surface because the Causal Edges density gate is unpassed (0 `intervention_tested` edges for any client). Three deviations from this section, each with a reason:
> - **Not a tree with toggleable overlays.** `kpi_relationship_basis_design.md` §5 (resolved later, 2026-08-21) rejected any fixed multi-panel layout: sections render *conditionally*, stacked. Assumptions is a per-card marker, never a section — §5: "Assumptions was never a fourth section to begin with."
> - **The Spine's chart is a variance bridge, not a composition tree.** §4 of that same note: "the framing question is 'why did this move' — inherently about the DELTA between periods, not the current value's arithmetic. A composition bridge doesn't answer that at all." The first build of the prototype *was* a composition bridge and was corrected before it landed.
> - **No dagre/elkjs needed.** This section assumed custom DAG layout; the real seeded tree is four nodes deep-two, so it lays out in CSS. "Static exhibit first" was followed — nothing interactive was built.
>
> What did carry through unchanged: **provenance encoding as the primary visual variable** (solid = identity or DiD-tested, dashed = asserted, dotted = template/external), and the discipline that numbers appear only on the arithmetic spine so the qualitative layers cannot borrow its authority.
