...
| `strategic_causal_graph_design.md` | **Design note, not built.** DQ link 1 (frame) failed 3/3 live runs on 2026-08-19 (2 MBB, 1 lens), identical reasoning each time. Reframed 2026-08-20 against `theory_layer_design.md` §2.2's own three layers (physical/operational, economic, commitment): today's causal graph is entirely layer-2 (economic) — the real problem isn't "no strategic option," it's that the graph cannot represent an alternative outside layer 2 at all. Two concrete gaps: layer 1 is entirely absent (a portfolio/capacity alternative — worked example: lubricants `commodity_grade_portfolio_participation`), and layer 3 exists today only as a passive constraint, never as an active lever (worked example: the price-lock covenant already in "Known Constraints," reframed as something a decision could target, not just obey). Proposes a distinct strategic-node model tagged by `theory_layer`, its own provenance vocabulary (the empirical `template→confirmed→hitl_proposed→va_validated` ladder doesn't fit a judgment no query can confirm), and — the highest-value consequence — converting L1 from a 71%-false-positive text screen into a real check of whether a cross-layer alternative was offered and given an explicit verdict. Curation resolved: LLM-drafted candidates (same `A9_KPI_Assistant_Agent` shape — suggest, refine, human-finalize), grounded in real DA/MA facts, never presented live/unsupervised |
