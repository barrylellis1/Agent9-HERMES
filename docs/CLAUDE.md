...
| `problem_framing_design.md` | **Design note, not built, no phase assigned.** DQ link 1 (appropriate frame) fails 11 of 13 scored runs and is the **only systematic cap left** on the chain. Verifiable cause: all **nine** refinement topics operate *inside* the frame — SA names a card after one breached KPI and no stage ever asks whether recovering it is the objective, which is why both the roster swap and the frame-challenge wording test returned nulls. Argues **against** computing a "perfect frame" (that reproduces Stage J's defect — a frame nobody chose — one layer up) and **for** a tenth `problem_framing` interview topic asked first, offering concrete alternatives built from the causal graph + assumption register. Re-reads the D-arm null: permission alone failed, permission *plus an upstream frame decision* was never tested. Turn-budget and sequence-cap risks are already handled by `effective_turn_budget()`; the unhandled one is that refinement is optional. Carries a stated falsifier |
