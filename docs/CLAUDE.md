...
| `strategic_causal_graph_design.md` | **Design note, not built.** DQ link 1 (frame) failed 3/3 live runs on 2026-08-19 (2 MBB, 1 lens) with identical reasoning each time — traced to the causal graph containing only operational/financial KPI-to-KPI edges, so a principal at the mandatory framing gate can never be offered a strategic/portfolio-participation alternative in the first place. Proposes a distinct strategic-node model (own provenance vocabulary — the empirical `template→confirmed→hitl_proposed→va_validated` ladder doesn't fit a judgment that no query can confirm), bespoke per-client curation, and — the highest-value consequence — converting L1 from a 71%-false-positive text screen into a real check of whether a structural alternative was offered and given an explicit verdict. Curation resolved 2026-08-20: LLM-drafted candidates (same `A9_KPI_Assistant_Agent` shape — suggest, refine, human-finalize), grounded in real DA customer-concentration data and MA competitive signals rather than freelanced, never presented live/unsupervised. Concrete worked example included (lubricants `commodity_grade_portfolio_participation`) |
